...
| `embedder.py` | Stage 2 of the agentic matching funnel: `embed_text()` (Gemini `gemini-embedding-001`, cached once per posting/profile), `cosine_similarity()`, `rerank_by_similarity()` — in-process, no pgvector/vector DB |
| `agent.py` | Stage 3 of the agentic matching funnel: LangGraph `StateGraph` tool-calling loop (`run_agent()`) over a bounded shortlist (~8 postings). Tools: `score_match`, `draft_bullet`, `search_job_board`, `finish`. Model: Groq `openai/gpt-oss-120b`. `MAX_ITERATIONS=3` hard cost ceiling |
