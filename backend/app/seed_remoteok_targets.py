"""Seed a single virtual target for the RemoteOK connector."""
import sys
...
def run():
    db = SessionLocal()
    connector = db.query(Connector).filter(Connector.name == "remoteok").first()
    if not connector:
        print("Connector 'remoteok' not found — run seed_connectors.py first"); db.close(); return
    ...
    db.commit()
    db.close()
    print(f"RemoteOK targets: {added} added")
